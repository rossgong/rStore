...
Product.prototype.toString = function () {
	return this.name + " (" + this.department + ") " + new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(this.price) + " [" + this.stock + " in stock]";
}
...
var sqlDB = function () {
	...
	this.addProduct = function (name, dep, price, stock) {
		this.connection.query(
			"INSERT INTO products SET ?",
			new Product(name, dep, price, stock).toSQL(),
			function (err) {
				if (err) throw err;
			}
		)
